...
ROOT_TRAIN_MODEL = "D:\\ADLA\\ADLA\\SavedModels\\Train1"
ROOT_PRETRAIN_MODEL = "D:\\ADLA\\ADLA\\SavedModels\\Pretrain1"
ROOT_INDICATORS = "D:\\ADLA\\ADLA\\SavedIndicators"
writer = SummaryWriter(log_dir="D:\\ADLA\\ADLA\\log")

#ROOT_DATA = "/mnt/sdb1/cxw/project2/cancer"
#ROOT_TRAIN_MODEL = "/mnt/sdb1/cxw/project2/SavedModels/Train"
#ROOT_PRETRAIN_MODEL = "/mnt/sdb1/cxw/project2/SavedModels/Pretrain"
#ROOT_INDICATORS = "/mnt/sdb1/cxw/project2/SavedIndicators"
#writer = SummaryWriter(log_dir="/mnt/sdb1/cxw/project2/log")
...
def get_augmented_features(data, cvae_model, concat, device):
    ...
    for _ in range(concat):  # 通过循环生成concat次增强特征
        z = torch.randn([data.x.size(0), cvae_model.latent_size]).to(device)  # 随机生成潜在变量 z
        augmented_features = cvae_model.inference(z, data.x).detach()
        augmented_features_list.append(augmented_features)
    return torch.cat(augmented_features_list, dim=1)  # 拼接增强特征
...
def run(cancer_name, data, lr):
    ...
    model = torch.load("{}\\{}ADLA_best_model.pth".format(model_path, cancer_name))
    ...
    #risk_df.drop()
    #risk_df.to_csv("risk_group_PRO2.csv", index=False, header=False)
    pass
