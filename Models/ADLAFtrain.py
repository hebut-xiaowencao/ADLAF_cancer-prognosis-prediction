...
def save_evaluation_indicators(indicators, model_name):
    ...
    path = "{}\\ADLAF\\{}_Indicators.xlsx".format(Indicators, model_name)
    file = open(path, "a")
...
def train(cancer_name, data, lr, value):
    ...
    for train_index, validate_index in kf.split(X=censored_data):
        ...
        wcvae_model = torch.load('{}\\{}\\{}_pretrain.pth'.format(Pretrain, cancer_name, cancer_name)).to(
            device=device)
        ...
        for i in range(60):
            ...
            with torch.no_grad():
                ...
                early_stopping(val_loss=1 - validate_Cindex, model=model,
                                        path="{}\\{}ADLAF_best_model.pth".format(model_path, cancer_name + str(num)))
    ...
    print("====================RESULT=======================")
    model_best = torch.load("{}\\{}ADLAF_best_model.pth".format(model_path, cancer_name + str(best_num)))
    model.eval()
