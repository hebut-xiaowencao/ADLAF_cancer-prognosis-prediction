...
#from Models.WCVAEplot import plot_loss_curve
...
def log_normal(x, mu, var):
    ...
    eps = 1e-8
    if eps > 0.0:
        var = var + eps #Avoid a variance of 0
...
def gaussian_loss(z, z_mu, z_var, z_mu_prior, z_var_prior):
    loss = log_normal(z, z_mu, z_var) - log_normal(z, z_mu_prior, z_var_prior)#计算KL散度
    return loss.mean() 
...
def generated_generator(args, device, adj_scipy, features, cancer_name):
    ...
    if not x_list or not c_list:
        raise ValueError("no avaliable features")
    ...
    writer = SummaryWriter(log_dir="/mnt/sdb1/cxw/data/log")
    ...
    for epoch in range(args.total_iterations):
        ...
        print(f"Epoch: {epoch}, BCE: {BCE}, KLD: {KLD}, Loss: {cvae_loss}")

        writer.add_scalar('cvae_loss', cvae_loss, epoch)
        ...
        model_path = f"D\\ADLA\\ADLA\\SavedModels/Pretrain1/{cancer_name}"
        if not os.path.exists(model_path):
            os.makedirs(model_path)
        early_stopping(cvae_loss.item(), cvae, path=f"{model_path}/{cancer_name}_pretrain.pth")
